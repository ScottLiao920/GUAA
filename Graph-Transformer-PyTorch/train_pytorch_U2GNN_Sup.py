...
class Batch_Loader(object):
    def __call__(self):
        ...
        input_x, graph_pool, X_concat, graph_labels = get_batch_data(
            batch_graph)
        return input_x, graph_pool, X_concat, graph_labels
...
def evaluate():
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(test_graphs), args.batch_size):
            ...
            test_input_x, test_graph_pool, test_X_concat, _ = get_batch_data(
                batch_test_graphs)
            prediction_scores = model(
                test_input_x, test_graph_pool, test_X_concat).detach()
...
out_dir = os.path.abspath(os.path.join(
    args.run_folder, "../runs_pytorch_U2GNN_Sup", args.model_name))
print("Writing to {}\n".format(out_dir))
...
for epoch in range(1, args.num_epochs + 1):
    ...
    write_acc.write('epoch ' + str(epoch) + ' fold ' +
                    str(args.fold_idx) + ' acc ' + str(acc_test*100) + '%\n')
    try:
        torch.save(model.state_dict(), os.path.join(out_dir, 'model', str(epoch)))
