...
from GraphTransformerPyTorch.util import *
...
parser = ArgumentParser(
    "U2GNN", formatter_class=ArgumentDefaultsHelpFormatter, conflict_handler='resolve')
...
parser.add_argument("--num_timesteps", default=1, type=int,
                    help="Timestep T ~ Number of self-attention layers within each U2GNN layer")
parser.add_argument("--ff_hidden_size", default=1024, type=int,
                    help="The hidden size for the feedforward layer")
parser.add_argument("--num_neighbors", default=4, type=int, help="")
parser.add_argument('--fold_idx', type=int, default=1,
                    help='The fold index. 0-9.')
...
def get_idx_nodes(selected_graph_idx):
    idx_nodes = [torch.where(graph_indices == i)[0]
...
def get_batch_data(selected_idx):
    ...
    X_concat = np.concatenate(
        [graph.node_features for graph in batch_graph], 0)
    if "REDDIT" in args.dataset:
    ...
    for input_node in range(X_concat.shape[0]):
        if input_node in dict_Adj_block:
            input_neighbors.append([input_node] + list(np.random.choice(
                dict_Adj_block[input_node], args.num_neighbors, replace=True)))
        else:
            input_neighbors.append(
                [input_node for _ in range(args.num_neighbors + 1)])
...
modelArguments = {
    ...
    'device': device}
model = TransformerU2GNN(**modelArguments).to(device)
# save model arguments for elegant model loder
with open(os.path.join(
            '/GUAA', 'models', args.dataset, 'modelArguments.pickle'), 'wb') as handle:
    pickle.dump(modelArguments, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
def evaluate():
    ...
    with torch.no_grad():
        ...
        graph_embeddings = torch.spmm(
            graph_pool, node_embeddings).data.cpu().numpy()
        acc_10folds = []
...
out_dir = os.path.abspath(os.path.join(
    args.run_folder, "../runs_pytorch_U2GNN_UnSup", args.model_name))
print("Writing to {}\n".format(out_dir))
...
for epoch in range(1, args.num_epochs + 1):
    ...
    write_acc.write('epoch ' + str(epoch) + ' mean: ' +
                    str(mean_10folds*100) + ' std: ' + str(std_10folds*100) + '\n')
    if mean_10folds >= 1.01 * best_acc:
        best_accu = mean_10folds
        es_cnt = 0
        if not os.path.exists(os.path.join('/GUAA', 'models', args.dataset)):
            os.mkdir(os.path.join('/GUAA', 'models', args.dataset))
        # try:
        #     os.remove(os.path.join('/GUAA', 'models',
        #               args.dataset, str(best_epoch)+'.pt'))
        #     best_epoch = epoch
        # except FileNotFoundError:
        #     pass
        # torch.save(model, os.path.join(
        #     '/GUAA', 'models', args.dataset, str(best_epoch)+'.pt'))
        torch.save(model.state_dict(), os.path.join(
            '/GUAA', 'models', args.dataset, 'model.pt'))
    else:
        es_cnt += 1
    if es_cnt == 15:
        break
